...
ggplot(dfArtisDuration_mean, mapping = aes(x = duration_mean_in_sec))+
   geom_histogram(mapping = aes(fill = ifelse(duration_mean_in_sec <= mean(duration_mean_in_sec),"En dessous de la moyenne","Au dessus de la moyenne")), binwidth = 5) +
   scale_fill_discrete(name ='Classification par la durée moyenne des musiques')+
...
data %>% select(track_popularity, duration_ms) %>% 
  ...
  xlab("Durée en secondes") +
  ylab("Popularité des Tracks en pourcentage")
...
ggplot(data_2000_2019, aes(x = track_album_release_year, y = total_streams, fill = playlist_genre)) +
  geom_area() +
