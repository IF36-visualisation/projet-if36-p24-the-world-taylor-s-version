---
title: "Rapport - Projet IF36"
author: "The World (Taylor's Version)"
output:
  html_document:
    toc: true
    toc_depth: 6
...
## Introduction & Données 💡 {#introduction}
...
Les features audio sont des données extraites des musiques elles-mêmes par l'API Spotify. Elles sont calculées par des algorithmes de traitement du signal audio. Ces algorithmes ne sont pas publics, nous ne pouvons donc pas faire d'analyse dessus, et cela peut être arbitraire, mais voici ce que Spotify dit de ces features :
...
```{r, echo=FALSE, fig.align='center', fig.height=6, fig.width=15}

# Graph of distribution of the number of musics per genre, ordered by the number of musics
g1 <- ggplot(data, aes(fill = playlist_genre, x = reorder(playlist_genre, -table(playlist_genre)[playlist_genre]))) +
  geom_bar() +
...
# Graph of distribution of the number of musics per year
g2 <- ggplot(data, aes(x = track_album_release_year)) +
  geom_bar() +
  ggtitle("Distribution du nombre de musiques par année") +
  xlab("Année") +
  ylab("Nombre de musiques") + 
  theme(plot.title = element_text(size = 20))
...
Lors de notre projet nous avons également utilisé **Shiny** afin de présenter quelques graphiques de façon dynamique et plus raffinée. 

L'application est dans le dossier *shiny* puis on peut lancer le ficher *app.R* : nous obtenons alors une application contenant 3 onglets.
Les deux premiers onglets permettent de répondre au quatres dernières questions de ce rapport, et le dernier onglet contient une petite galerie d'image de *Taylor Swift*.

![ScreenShot shiny galerie](image/shiny_galerie.png)
...
ggplot(long_data, aes(y = Streams, x = fct_reorder(Artist, Streams, sum), fill = Platform)) + 
  ...
  scale_fill_manual("Plateforme", values = c("Spotify" = "#1DB954", "YouTube" = "#FF0000"))+  theme(plot.title = element_text(size = 11))
...
ggplot(data, aes(x = energy, y = total_streams)) +
  geom_smooth() +
  labs(x = "Energie", y = "Streams totaux", title = "Correlation entre énergie et streams totaux") +
  theme_minimal() +
  scale_y_continuous(labels = scales::comma)
...
Pour pousser un peu l'analyse, nous avons également étudié la moyenne mobile du nombre de streams en fonction de l'energy pour chaque genre.

```{r}
ggplot(data, aes(x = energy, y = total_streams, color = playlist_genre)) +
  geom_smooth(method = "loess", se = FALSE) +
  ggtitle("Evolution de la durée des chansons au fil du temps, pour quelques artistes") +
  labs(x = "Energie", y = "Streams totaux", title = "Scatter Plot : Energie et moyenne de streams par genre") +
  theme_minimal() +
  scale_y_continuous(labels = scales::comma)
```

On remarque que l'EDM explose aux alentours de 0.3 d'energy, cela est dû à 2 outliers qui poussent cette moyenne vers le haut, car il y a très peu de titre EDM à ce niveau d'energy.

La variation positive évoquée sur le graphique précédent entre 0.25 et 0.75 peut cependant être confirmée sauf pour le rap et R&B. Cela peut être compréhensible, une personne écoutant du rap et du R&B aura des attentes plutôt basées sur les paroles de la musique qu'une forte energy (qui on le rappelle est calculée subjectivement par Spotify mais qui comprend notamment le tempo d'une musique ainsi que les instruements utilisées).
...
radarchart(top_musiques_norm, 
           ...
           plty = 1, 
           plwd = 2,
           title = "Radar Chart caractéristiques du top 5 musiques",
           axislabcol = "white",
           cglcol = "grey",
           cglty = 1,
           cglwd = 0.8,
...
Cette question est surtout traitée par curiosité, les données du radar chart sont analysées entre elle avec les extrêmes calculés sur les plus hautes et basses valeur de ces 5 musiques. 
...
##### **Est-ce que les musiques qui ont le plus de "Views" sur Youtube sont également celle qui ont le plus de "Stream" sur Spotify ? **

##### **Dans la même idée, est-ce que les musiques qui ont le plus de "Views" sont également celle qui ont le plus de "Likes" et de "Comments" ?**
...
(Vous pourrez également profiter d'une image exclusive de Taylor Swift lors d'un de ses concert 2024 en France ! )
...
##### **Existe-t-il des liens entre certaines variables et le nombre de streams total ?**
