...
ggplot(dfArtisDuration_mean, mapping = aes(x = duration_mean_in_sec)) +
  geom_histogram(mapping = aes(
    fill = ifelse(duration_mean_in_sec <= mean(duration_mean_in_sec), "Below Average", "Above Average")
  ), binwidth =5) +
  scale_fill_manual(
...
# For the second plot
g2 <- ggplot(head(dfArtisDuration_infEg_mean, 10), aes(y = duration_mean_in_sec, x = Artist, fill = Artist)) +
  geom_bar(position = "stack", stat = "identity") +
  geom_hline(yintercept = mean_in_s) +
  annotate("text", x = -1, y = mean_in_s + 10, label = 'Mean', hjust = 'left') +
  geom_text(aes(label = round(duration_mean_in_sec, 2), angle = 70), position = position_stack(vjust = 1.1)) +
...
> Est ce que l'energy d'une musique joue un rôle important dans sa popularité ?
